...
const startCommand = async (msg, bot) => {
  ...
  bot
    .sendMessage(
      msg.chat.id,
      "Please Login To Your Trello Account ,By clicking <a href='https://trello.com/en/login'>Here</a> . Go <a href='https://trello.com/power-ups/admin/'>Here</a> Then Accept Agreement . Then Click <a href='https://trello.com/app-key'>Here</a> To Get <b>Personal App Key</b> And Reply Key By Selecting this message to proceed .",
      {
        parse_mode: "HTML",
      }
    )
          ...
          .then((tokenMessage) => {
            bot.onReplyToMessage(msg.chat.id, tokenMessage.message_id, async (token) => {
              try {
                ...
                bot.sendMessage(msg.chat.id, "Telgram Bot Mai Aapka Swaagat Hai.");
              } catch (e) {
                // Catching Error's
